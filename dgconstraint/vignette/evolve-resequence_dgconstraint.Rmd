...
* Before you start the analysis, you need to ensure that:
  ...
  + You have `data-in` and `data-out` folders in your working directory: `data-in` will contain the data to analyze (i.e., the .csv files), analysis will be written to the `data-out` folder
...
### Step 2: Organizing the wild data

Not all of the data that you will come across will fit the function requirements. 

#### There must be a column labeled **Gene** in the data file

#### Mutation presence/absence must be indicated by 1/0
The functions require that mutation presence and absence is recorded numerically by 1 (presence) or 0 (absence).
...
There are other ways to code the data to look the way we want. This is a basic way to do it, but other functions will work as well. You might have to do this for datasets of the other types of function, but it is all the same idea, refer to the types of functions below to see how the data should look like.    
...
* *multiple_wide(paper, environment, species, generations)* is the function we are using:    
...
*multiple_wide("Author2014", "YPD", "Sac", c("0", "140", "240", "335", "415", "505", "585", "665", "745", "825", "910", "1000"))*  

### Results  
The package will give you the results in a table with all the information and calculations such as the paper, environment, generations, c-hyper, p-value, estimate, number of populations, number of non parallel genes, number of parallel genes and a list of the parallel genes.  
...
Now we have to fill in the parameters in the function: *multiple_long(paper, environment, species, selective_pressure)*  
If we use the data above as an example, then the function would look like this:  
*multiple_long("Author2017", "YPD", "Ecoli_K12", c("OT", "HT"))*   

### Results  
The package will give you the results in a table with all the information and calculations such as the paper, environment, selective_pressure, c-hyper, p-value, estimate, number of non parallel genes, number of parallel genes and a list of the parallel genes.    
...
Now we have to fill in the parameters in the function: *single_wide(paper, environment, species, population)*
If we use the data above as an example, then the function would look like this:  
*single_wide("Author2014", "glucose minimal media", "Ecoli_K12", c("ALE1", "ALE2", "ALE3", "ALE4", "ALE5", "ALE6", "ALE7", "ALE8", "ALE9", "ALE10"))*  
...
Now we have to fill in the parameters in the function: *single_long(paper, environment, species)*  
If we use the data above as an example, then the function would look like this:  
*single_long("Author2018", "YPD", "Ecoli_K12")*  
