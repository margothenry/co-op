...
#################################################################################################
data <- read_csv("data-in/McCloskey2018.csv")
...
if (multiple == TRUE) {
  ...
  for (g in generations) {
   #####################################################################################################################
    ...
    num_genes <- length((unique(data.1$Genes)))
    
    #number of lineages
    num_lineages <- length(unique(data.1$Population))
    
    #making the matrix
    data.array <- array(0, dim =c(num_genes, num_lineages), dimnames = list(unique(data.1$Genes), unique(data.1$Population)))
    
    for(i in 1:num_lineages) {
      sub <- subset(data.1, data.1$Population == unique(data.1$Population)[i])
      sub2 <- subset(sub, frequency > 0)
      geneRows <- which(row.names(data.array) %in% sub2$Genes)
  ...
  data.1 <- data %>%
    ...
    drop_na(Genes) %>%
    drop_na(Population) %>% 
    filter(mutation != "MOB", mutation != "INS", mutation !="AMP") %>% 
